...
\begin{denotation}[3cm]
  \item[$q_{\phhi}(z|x)$] 表示VAE模型中的编码器，其中$\phi$为编码器的参数
  ...
  \item [$p(\boldsymbol{x}_t\mid y)$] 条件似然函数，用于生成条件生成中需要拟合的新的Score function
  \item [$\nabla_{\boldsymbol{x}_t}\log\left(p(\boldsymbol{x}_t\mid y)\right)$] 条件生成问题下的Score function
  \item [Score matching] 
